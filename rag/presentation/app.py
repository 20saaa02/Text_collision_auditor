...
class AnticollisionMainClass:
    ...
    def prepareDB(self, getData: prepareDBGet) -> prepareDBResult:
        ...
        """
        # ФАКТЫ В ЭМБЕДДИНГИ
        factsIntoEmbeddingsRes = self.factsIntoEmbeddings(factsIntoEmbeddingsGet(getData.text))
        if factsIntoEmbeddingsRes.error.isError:
            prepareDBRes.error = factsIntoEmbeddingsRes.error
            return prepareDBRes
        # ЭМБЕДДИНГИ ЗАПИСЫВАЕМ В БАЗУ
        loadEmbeddingsDBRes = self.RAGRetriever.loadEmbeddingsDB(loadEmbeddingsDBGet(sentence_embeddings=factsIntoEmbeddingsRes.embeddings, sentences=getData.text))
    ...
    def checkCollisionOne(self, getData: checkCollisionOneGet) -> checkCollisionOneResult:
        ...
        findTopNearestDBRes = self.RAGRetriever.findTopNearestDB(findTopNearestDBGet(query_embedding=factsIntoEmbeddingsRes.embeddings[0], k=COUNT_NEAR_FIND_DB,\
